...
func Server(conn net.Conn, config *Config) *Conn {
	if config != nil && os.Getenv("TLS_GM_SUPPORT") == "true" {
		fmt.Println("server enable gm handshake connecting....", true)
		config.GMSupport = &GMSupport{}
		config.MinVersion = config.GMSupport.GetVersion() //only gm support VersionGMSSL
		if len(config.CipherSuites) != 0 {
			config.CipherSuites = append(config.CipherSuites, GMTLS_SM2_WITH_SM4_SM3)
			config.CipherSuites = append(config.CipherSuites, GMTLS_ECDHE_SM2_WITH_SM4_SM3)
		}
	} else if config != nil && config.GMSupport != nil {
		//fabric go sdk 启动多实例，优化掉环境变量
		fmt.Println("server enable gm handshake connecting....", true)
		config.MinVersion = config.GMSupport.GetVersion() //only gm support VersionGMSSL
		if len(config.CipherSuites) != 0 {
			config.CipherSuites = append(config.CipherSuites, GMTLS_SM2_WITH_SM4_SM3)
...
func Client(conn net.Conn, config *Config) *Conn {
	if config != nil && os.Getenv("TLS_GM_SUPPORT") == "true" {
		fmt.Println("client enable gm handshake connecting....", true)
		config.GMSupport = &GMSupport{}
		config.MinVersion = config.GMSupport.GetVersion() //only gm support VersionGMSSL
		if len(config.CipherSuites) != 0 {
			config.CipherSuites = append(config.CipherSuites, GMTLS_SM2_WITH_SM4_SM3)
...
func parsePrivateKey(der []byte) (crypto.PrivateKey, error) {
	...
	if key, err := x509.ParsePKCS8PrivateKey2(der); err == nil {
		switch key := key.(type) {
